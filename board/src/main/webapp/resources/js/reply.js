...
let replyService=(function(){
	...
	function displayTime(timeValue){
		...
		// ms ==> 변환
		// 댓글 단 날짜가 오늘(24hr 기준)이면 시분초로 나오게
		// 댓글 단 날짜가 오늘이 아니면 년월일로 나오게
...
$(function(){
	...
	$("#modalRegisterBtn").click(function(){
		let reply = {
			bno:bno,reply:modalInputReply.val(),replyer:modalInputReplyer.val()
			};
	...
	function showList(page){
		...
		// page:page||1 : page 변수 값이 들어오면 사용하고 안들어오면 1로
	...
	// 댓글 클릭 이벤트 - 이벤트 위임(chat을 이용해 이벤트 걸고 실제 이벤트 대상은 li 태그가 되는 형태)
	$(".chat").on("click","li",function(){ // 사실 이렇게 on 주고 클릭 하는데 클릭만 하는 것의 기본 형태
