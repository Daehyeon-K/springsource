	...
	$(function(){
		$("#all").click(function(){
			...
			/* $.ajax({
				...
				dataType:'json',
				success:function(){
					// 서버의 응답코드(Http 상태코드)가 200일 때 	
				}
			}) */
			
			// 가져올 데이터가 json이라면 아래가 더 편함
			$.getJSON({
				url:'list',
		...
		$("#delete").click(function(){
			$.ajax({
				url:'4003',
				type:'delete', // delete 매핑 부름 (@DeleteMapping과 연결)
				success:function(data){
					alert(data);
				},
		...
		$("#update").click(function(){
			...
			$.ajax({
				url:'3001',
				type:'put',
				contentType:'application/json', // 이거 안하면 이 에러 => 'application/x-www-form-urlencoded;charset=UTF-8' not supported
				data:JSON.stringify(param), // let param으로 그냥 쓴건 자바스크립트라 JSON으로 만들어주는
				success:function(data){
					alert(data);
				},
				error:function(xhr,status,error){
